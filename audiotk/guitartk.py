...
def rand_fretboard_position(start=1, end=6):
    ...
    while True:
        print(f'string: {random.randint(start, end):02} fret:{random.randint(0, 12):02}', end='\r')
        time.sleep(4)


def rand_loop(notes, rest: Optional[float] = None) -> None:
    while True:
        print(random.choice(notes))
        time.sleep(rest) if rest else input('')


def rand_note(rest: float = 5.):
...
def main():
    fire.Fire({
        'fb_pos': rand_fretboard_position,
        'rand_note': rand_note,
    })
