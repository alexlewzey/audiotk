...
def tag_sample(sample: Path) -> str:
    """assign a sample file to a folder eg (kick, snare etc) returning folder as a string"""
...
def plot_wave(path: Path, title: Optional[str] = None) -> Tuple:
...
def user_tag_check(cwd: Path, sample: Path) -> Optional[str]:
    """allows the user to visually and audibly check the destination dir assigned to the sample and
     override it if required"""
    dirs = [path for path in cwd.iterdir()
    ...
    while True:
        dest = input(f'Destinations:\n{dirs}\n\n{sample.name} to [{file_tag}]: ') or file_tag
        if dest == 'r':
            play(clip)
            continue
        if dest == 's':
...
def move_sample(cwd: Path, sample: Path, dest_dir: str) -> None:
    ...
    try:
        ...
        path_duplicate: Path = dest_path.parent / (dest_path.stem + '_dub' + dest_path.suffix)
...
def is_sample_file(sample: Optional[Path]) -> Optional[bool]:
    if sample:
...
def allocate_sample(cwd: PathOrStr, sample: Path) -> None:
    """assigen sample to folder > get user to check destination with option to override
...
def organiser(cwd: PathOrStr) -> None:
    """"""
    cwd = Path(cwd)
